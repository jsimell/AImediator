/* 
  This file is meant for testing, removing the need to manually fill in the initial prompt form.
  Change the InitialPrompForm component in App.jsx to this component to use the pre-filled form.
*/

import { useState } from 'react';
import taskService from '../services/task'

const FilledInitialPromptForm = ({ setDiscussionInfo, participants, setParticipants, formDisabled, setFormDisabled, setIsLoading, addMessage }) => {

  const [discussionTopic, setDiscussionTopic] = useState("Energy drinks");              // The topic of the discussion in a few words
  const [participantOpinions, setParticipantOpinions] = useState(["I think energy drinks are a great invention because I can use them to give me an energy and focus boost whenever I need it. E.g. while studying", "I think that humans are not meant to drink a processed liquid like energy drinks, and I think that Jiri drinks them way too often."]);
  const [participantGoals, setParticipantGoals] = useState(["I want Pekka to understand why I drink energy drinks, and what are their benefits for me.", "I want Jukka to understand that drinking them too much can have serious negative effects on his health."]);
  const [participantRelationship, setParticipantRelationship] = useState("We are friends");
  const [alreadyDiscussed, setAlreadyDiscussed] = useState("We have discussed the topic previously on multiple occasions, but have never reached an agreement.");
  const [hasDiscussedBefore, setHasDiscussedBefore] = useState(true);
      ...
      <div className="initial-prompt-form-wrapper">
        <form onSubmit={chooseDiscussionInformation} className="initial-prompt-form">
          ...
          {hasDiscussedBefore && (<div className="input-group">
            <label htmlFor="discussed">What have you discussed?</label>
            <textarea
              id="discussed"
              className="input-field"
              type="text"
...
export default FilledInitialPromptForm;
